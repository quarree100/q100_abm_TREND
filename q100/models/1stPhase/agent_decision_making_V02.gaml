...
*
*
AUS UNERFINDLICHEN GRUENDEN WIRFT DIESE VERSION FEHLERHAFTE WERTE AUS -> AUFNAHME VON V03
*
*
*
*
...
// for choosing specific value -> [rows, columns]	
	...
	int tra_lala <- 1; //TEST2 ??? (2021-11-24_12:00)
	...
	float employment <- share_employment_income[1];
	
	float ownership <- share_ownership_income[1];
		

	init { //erster Versuch der Integration der Zahl der Haushalte; Schritt 2 -> Ausrichten an Anzahl der im GIS-Datensatz hinterlegten Wohnungen
		...
		// Age (Employment & Ownership is distributed by income groups)
		ask (share_age_21_40 * nb_units) among households {
			age <- rnd (21, 40);	
		}
	...
	output {
		...
		display "charts" {
			chart "households" type: histogram {
				...
				data "total" value: sum (households_500_1000 count (each.income >= 0), households_1000_1500 count (each.income >= 0), households_1500_2000 count (each.income >= 0), households_2000_3000 count (each.income >= 0), households_3000_4000 count (each.income >= 0), households_4000etc count (each.income >= 0)) color:#black;
			}
		}
	}
}